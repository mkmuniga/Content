...
def upload_seq_trk(input_file_path, output_dir):
    schema_overrides = {
        "ADDRESS": field(name='ADDRESS'),
        "TIME": field(name='TIME', type=FieldType.UINT),
    }
    schema = discover_schema(file_path=input_file_path, schema_overrides=schema_overrides)
...
def match_and_upload(input_file_path, output_dir, utdb_name):
    ...
    pattern_to_uploader_dict = {
        '^sequence_trk_uvm.log*$': upload_seq_trk,
    }
