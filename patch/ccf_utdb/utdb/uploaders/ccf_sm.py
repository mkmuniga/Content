...
def upload_sm(input_file_path, output_dir):
    schema = table_schema()
    schema.add_record_schema('SM', [
        field(name='MEM_NAME', type=FieldType.STRING),
        field(name='REGION_NAME', type=FieldType.STRING),
        field(name='SUB_NAME', type=FieldType.STRING),
        field(name='LOW_ADDRESS'),    #, type=FieldType.UINT, format='{:014X}', null_value='-'
        field(name='HIGH_ADDRESS'),   #, type=FieldType.UINT, format='{:014X}', null_value='-'
        field(name='SIZE', type=FieldType.INT),
    ])

    reader = Reader(file_path=input_file_path, schema=schema, separator='|', include='(^DRAM|^MMIO|^REMAP_AREA|^LEGACY)'
