...
def upload_register(input_file_path, output_dir):
    schema = table_schema()
    schema.add_record_schema([
        field(name='TIME', type=FieldType.UINT),
        field(name='UNIT', type=FieldType.STRING),
        field(name='SIGNAL_PATH', type=FieldType.STRING),
        field(name='_TYPE', type=FieldType.STRING),
        field(name='REG', type=FieldType.STRING),
        field(name='FIELD', type=FieldType.STRING),
        field(name='VALUE'),
    ])
    
    
    selector_field_index = 3
...
def match_and_upload(input_file_path, output_dir, utdb_name):
    ...
    pattern_to_uploader_dict = {
        '^register_cbo_trk.log*$': upload_register,
    }
